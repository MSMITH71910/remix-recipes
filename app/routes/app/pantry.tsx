...
import { createGroceryItem } from "~/models/grocery-item.server";
import {
  createShelf,
  deleteShelf,
  getAllShelves,
  ...
  switch (formData.get("_action")) {
    ...
    case "createShelfItem": {
      return validateForm(
        ...
        (data) => createShelfItem(user.id, data.shelfId, data.itemName, data.itemQuantity),
        (errors) => data({ errors }, { status: 400 })
      );
    }
    case "deleteShelfItem": {
      return validateForm(
        ...
        async (parsedData) => {
          ...
          // Before deleting, add the item to grocery list for restocking
          if (item !== null && item.shelfId) {
            try {
              // Get the shelf name to determine source
              const shelf = await db.pantryShelf.findUnique({
                where: { id: item.shelfId },
                select: { name: true }
              });
              
              const source = shelf?.name.toLowerCase() || "pantry";
              await createGroceryItem(user.id, item.name, source, item.quantity);
            } catch (error) {
              console.error("Failed to add item to grocery list:", error);
              // Continue with deletion even if grocery item creation fails
            }
          }
    ...
    >
      ...
      <createShelfItemFetcher.Form
        ...
        onSubmit={(event) => {
          ...
          const itemQuantityInput = target.elements.namedItem(
            "itemQuantity"
          ) as HTMLInputElement;
          ...
          <div className="flex space-x-2">
            ...
            <input
              ...
              className={classNames(
                "w-24 outline-none",
                "border-b-2 border-b-background focus:border-b-primary",
                createShelfItemFetcher.data?.errors?.itemQuantity
                  ? "border-b-red-600"
                  : ""
              )}
            />
    ...
    <li className="py-2">
      <deleteShelfItemFetcher.Form method="post" className="flex">
        <p className="w-full">
          {shelfItem.name}
          {shelfItem.quantity && (
            <span className="text-gray-500 text-sm ml-2">({shelfItem.quantity})</span>
          )}
        </p>
