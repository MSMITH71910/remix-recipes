...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (recipe.userId !== user.id) {
    throw new Response("You are not authorized to view this recipe", { status: 401 });
  }
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  switch (_action) {
    case "addToGroceryList": {
      // Add recipe ingredients to grocery list if not in pantry/refrigerator
      const recipe = await db.recipe.findUnique({
        where: { id: recipeId },
        include: {
          ingredients: {
    ...
    case "deleteRecipe": {
      ...
      try {
        ...
        return data({ error: "Failed to delete recipe" }, { status: 400 });
      }
    }
    default: {
      return null;
      ...
      <Form method="post" encType="multipart/form-data" reloadDocument>
        ...
        <div className="flex mb-2">
          ...
          <button
            ...
            name="_action"
            value="deleteRecipe"
            className="flex flex-col justify-center ml-3 text-red-600 hover:text-red-800 p-2 rounded touch-manipulation min-h-[44px] min-w-[44px] items-center"
            onClick={(e) => {
              if (!confirm(`Are you sure you want to delete "${data.recipe?.name}"? This cannot be undone!`)) {
                e.preventDefault();
              }
            }}
          >
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            >
              <div className="flex flex-col justify-center h-full">📝 Add to Grocery List</div>
            </PrimaryButton>
